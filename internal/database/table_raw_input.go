...
func BuildTableDataQuerySQL(dbType, dbName, table, rawInput string, page, pageSize int, serverVersion string) string {
	...
	return fmt.Sprintf("SELECT * FROM %s WHERE %s LIMIT %d OFFSET %d", qt, in, pageSize, offset)
}
